...
public class EmojiParser {
    ...
    public static String parseToAliases(
        ...
        final EmojiTransformer emojiTransformer = unicodeCandidate -> {
     ...
     */
    public static String replaceAllEmojis(final String str, final String replacementString) {
        final EmojiParser.EmojiTransformer emojiTransformer = unicodeCandidate -> replacementString;
     ...
     */
    protected static AliasCandidate getAliasAt(final String input, final int start) {
        if (input.length() < start + 2 || input.charAt(start) != ':') return null; // Aliases start with :
        final int aliasEnd = input.indexOf(':', start + 2);  // Alias must be at least 1 char in length
        if (aliasEnd == -1) return null; // No alias end found

        final int fitzpatrickStart = input.indexOf('|', start + 2);
        if (fitzpatrickStart != -1 && fitzpatrickStart < aliasEnd) {
            final Emoji emoji = EmojiManager.getForAlias(input.substring(start, fitzpatrickStart));
            ...
            final Fitzpatrick fitzpatrick = Fitzpatrick.fitzpatrickFromType(input.substring(fitzpatrickStart + 1, aliasEnd));
            return new AliasCandidate(emoji, fitzpatrick, start, aliasEnd);
        }

        final Emoji emoji = EmojiManager.getForAlias(input.substring(start, aliasEnd));
        if (emoji == null) return null; // Not a valid alias
     ...
     */
    protected static AliasCandidate getHtmlEncodedEmojiAt(final String input, final int start) {
        ...
        final char[] chars = new char[EmojiManager.EMOJI_TRIE.maxDepth];
        int charsIndex = 0;
        int codePointStart = start;
        do {
            final int codePointEnd = input.indexOf(';', codePointStart + 3);  // Code point must be at least 1 char in length
            if (codePointEnd == -1) break;

            try {
                final int radix = input.charAt(codePointStart + 2) == 'x' ? 16 : 10;
                final int codePoint = Integer.parseInt(input.substring(codePointStart + 2 + radix / 16, codePointEnd), radix);
                charsIndex += Character.toChars(codePoint, chars, charsIndex);
            } catch (final IllegalArgumentException e) {
                break;
            }
            final Emoji foundEmoji = EmojiManager.EMOJI_TRIE.getEmoji(chars, 0, charsIndex);
            if (foundEmoji != null) {
    ...
    public static String parseFromUnicode(
        ...
        int prev = 0;
        final StringBuilder sb = new StringBuilder(input.length());
        final List<UnicodeCandidate> replacements = getUnicodeCandidates(input);
        for (final UnicodeCandidate candidate : replacements) {
    ...
    public static List<String> extractEmojis(final String input) {
        final List<UnicodeCandidate> emojis = getUnicodeCandidates(input);
     ...
     */
    protected static List<UnicodeCandidate> getUnicodeCandidates(final String input) {
        final char[] inputCharArray = input.toCharArray();
        final List<UnicodeCandidate> candidates = new ArrayList<>();
     ...
     */
    protected static UnicodeCandidate getNextUnicodeCandidate(final char[] chars, final int start) {
        for (int i = start; i < chars.length; i++) {
            final int emojiEnd = getEmojiEndPos(chars, i);

            if (emojiEnd != -1) {
                final Emoji emoji = EmojiManager.getByUnicode(new String(chars, i, emojiEnd - i));
                final String fitzpatrickString = (emojiEnd + 2 <= chars.length) ?
     ...
     * found
     */
    protected static int getEmojiEndPos(final char[] text, final int startPos) {
        int best = -1;
        for (int j = startPos + 1; j <= text.length; j++) {
            final EmojiTrie.Matches status = EmojiManager.EMOJI_TRIE.isEmoji(text, startPos, j);
    ...
    protected static class AliasCandidate {
        ...
        private AliasCandidate(final Emoji emoji, final Fitzpatrick fitzpatrick, final int startIndex, final int endIndex) {
            this.emoji = emoji;
